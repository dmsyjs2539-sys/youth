...
- 인트로 심볼은 파트별 회전이 필요해 index.html에 인라인 SVG로 삽입
  ...
  로고타입은 assets/icons/logo_intro_wordmark.svg (헤더 로고에서 심볼 제외 후 crop)
...
- 확인 내용:
  ...
  - 인트로 시퀀스: 200ms 바디 등장 → 700ms 회전 중 → 1300ms 정위치 수렴 →
    1800ms 로고타입 0.75 → 2600ms 오버레이 수축 → 3400ms DOM 제거·스크롤 잠금 해제
