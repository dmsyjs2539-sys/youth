# 미래청년비전연구소 현재 상태

마지막 업데이트: 2026-08-07

## 구현 완료

- 메인페이지 (Figma node 34:517 기준)
- 브랜드(연구소) 소개 페이지 `brand/index.html` (Figma node 34:679 기준)
  - 히어로: 배경 이미지 + eyebrow / 그라데이션 기관명 / 소개 문구
  - 철학: "Beyond Meta-Cognition, Towards Youth Future" + Vision(좌) / Mission(우)
  - Our Core Values: 카드 4종 (Self Awareness / Metacognition / Growth / Connection)
  - Our History: 중앙 라인 좌우 교차 타임라인 5개 (2019.05 ~ 2024.02)
  - 메인페이지의 헤더·GNB·푸터·맨 위로 버튼과 스크롤/스플릿 모션을 그대로 재사용
- 전체 반응형 (Mobile ~767px / Tablet 768~1023px / Desktop 1024px~)
  - 공통·메인페이지 반응형: `css/responsive.css`
  - 브랜드 페이지 반응형: `css/brand.css` 하단
  - 데스크톱에서 `--content_max: 1200px`로 최대 폭 제한 + 중앙 정렬
- 로그인 `login.html` / 회원가입 `signup.html` (공용 `css/auth.css`, `js/auth.js`)
  - 각 페이지에 카드 1개만 두고, 카드 하단 텍스트 링크로 서로 이동한다
  - 헤더 '회원가입' 버튼과 로그인 카드 하단 '회원가입하기'가 모두 signup.html로 연결
  - 로그인은 데모 고정 계정과 대조해 통과 시 research.html로 이동 (아래 '데모 로그인' 참고)
  - 회원가입은 인증 서버가 없어 제출하지 않고 안내 문구만 노출.
    문구는 각 폼의 `data-pending-message` 속성에서 읽는다
- 강의 및 컨설팅 절차 `program.html` (+ `css/program.css`, `js/program.js`)
  - Figma node 38:1712 기준. 히어로 + 세부 메뉴 탭 3개
  - 세부 메뉴: 강의 절차 / 컨설팅 절차 / 컨설팅 신청 (탭 전환, `#tab_*` 딥링크 지원)
  - 절차는 각 5단계 세로 타임라인. Figma에 있던 '신청서 작성' 1단계만 원문 그대로 쓰고
    나머지 단계 문구는 새로 작성함
  - 컨설팅 신청서는 접수 서버가 없어 제출을 막고 안내 문구만 노출
- 연구 자료 상세 `research.html` (+ `css/research.css`, `js/research.js`)
  - 로그인한 사용자만 볼 수 있는 임시 레이아웃. 본문·첨부는 예시 내용
  - 헤더 우측이 로그인/회원가입 대신 [이메일 + 로그아웃]으로 바뀐다
- GNB 3분할 개편 + hover 인터랙션 (2026-08-07)
- 헤더: 고정 헤더, 스크롤 시 배경 강화, 햄버거 메뉴 토글
- 히어로: 배경 이미지 + 하단 그라데이션 오버레이, 메인 카피
- 인트로: eyebrow 텍스트 + 그라데이션 기관명
- 메뉴 카드: 2x2 그리드 4종 (연구소 소개 / 연구 자료 / 교류 기관 / 강의 및 컨설팅 신청)
- 기관 소개: 배경 이미지 위 소개 문구
- 연구 분야: 카드 4종 (교육 연구 / 프로그램 개발 / 실험 및 검증 / 네트워킹)
- 관련 시스템 링크 2종, 푸터
- JS 애니메이션/인터랙션 (Vanilla JS + CSS Transition + IntersectionObserver)
  - 햄버거 아이콘 ↔ X 모핑 (열림/닫힘 역순)
  - 메뉴 항목 Stagger 페이드인 (65ms 간격)
  - 히어로 타이틀 글자 단위 스플릿 등장 (페이지 로드 시)
  - 섹션 타이틀 글자 단위 스플릿 등장 (스크롤 진입 시)
  - 카드·콘텐츠 Slide-Up & Fade-In (형제 요소 간 90ms Stagger)
- 진입 인트로(스플래시) 오버레이
  - 심볼(마름모 / 바디)이 각자 회전 + Glow하며 정위치 안착 → 반짝임 스윕
  - 로고타입 + 영문 태그라인 아래에서 위로 페이드인
  - 딥 네이비 오버레이가 중앙으로 수축(clip-path circle)하며 메인으로 전환
  - 종료 후 DOM에서 완전 제거, 스크롤 잠금 해제
- 맨 위로 이동 버튼 (우측 하단 고정, 스크롤 520px 초과 시 등장, smooth scroll)

## 구현 중

- 없음

## 확정된 UX 정책

- 전체 메뉴는 닫힌 상태로 시작하며, 메뉴 항목 클릭 또는 Escape 키로 닫힘
- 메뉴 버튼은 aria-expanded / aria-label을 상태에 맞게 갱신
- GNB 전환 기준은 **768px** (JS `INLINE_NAV_MIN_WIDTH`와 CSS 미디어쿼리가 같은 값)
  - ~767px: 로고 / [계정 아이콘 + 햄버거]. 전체 메뉴는 헤더 아래 드롭다운
  - 768px~: [좌] 로고 / [중앙] 전체 메뉴 / [우] 로그인·회원가입.
    햄버거와 계정 아이콘은 숨기고 메뉴는 항상 펼쳐 둔다
  - JS `syncNavMode()`가 hidden 속성과 열림 상태를 breakpoint에 맞춰 초기화하며,
    `setNavOpen()`은 768px 이상에서 아무 것도 하지 않는다
- 768~1023px는 가로 폭이 빠듯해 로고 **글자를 숨기고 심볼만** 노출한다.
  접근 가능한 이름은 `.header_logo`의 aria-label이 유지한다. 1024px부터 글자 복귀
- 로그인·회원가입 진입점은 모바일 계정 아이콘 / 768px 이상 텍스트 링크 두 가지
- 브랜드 소개 페이지에는 진입 인트로(스플래시)를 넣지 않는다.
  인트로는 메인페이지 진입 시에만 재생한다. (main.js는 splash 요소가 없으면 건너뜀)
- 브랜드 소개 페이지의 GNB '연구소 소개' 항목은 aria-current="page"로 현재 위치 표시
- 등장 애니메이션은 1회만 재생 (등장 후 observer unobserve)
- prefers-reduced-motion 설정 시 모든 모션을 끄고 콘텐츠를 즉시 노출
- 인트로는 메인페이지 로드 시마다 재생 (약 3.3초). 세션당 1회로 바꾸려면
  runSplash()에 sessionStorage 가드를 추가하면 된다.
- 인트로가 떠 있는 동안 body 스크롤을 잠그고, 히어로 타이틀 모션은
  인트로가 걷힌 뒤에 시작한다 (인트로 뒤에서 재생돼 버리는 것을 방지)

## 절차 페이지 구현 규칙 (2026-08-07)

- 세부 메뉴는 탭이다. `role="tablist"` / `role="tab"` / `role="tabpanel"`을 쓰고
  좌우 화살표·Home·End로 이동한다. 선택 안 된 탭은 `tabindex="-1"`.
- **숨김(`is_hidden`)은 JS가 붙인다.** HTML에는 세 패널이 모두 보이는 상태로 두어,
  JS가 없거나 실패해도 절차와 신청서를 읽을 수 있다.
- 탭 클릭 시 `history.replaceState`로 `#tab_*`을 남긴다.
  `program.html#tab_apply`처럼 특정 탭을 바로 열 수 있다.
- 타임라인 세로선은 `.step_list::before`이며, 아이콘 원의 **정중앙**을 지나야 한다.
  아이콘 크기를 바꾸면 `left` 값도 같이 조정할 것
  (모바일 원 56px → left 27px, 태블릿 원 64px → left 31px).
- 절차 아이콘(`assets/icons/icon_step_*.svg`)은 32×32 / stroke 1.6으로 기존 아이콘 규격을
  따르되, `<img>`로 불러 쓰므로 `currentColor`가 먹지 않아 브랜드색 `#2ba1ff`를 파일에 박았다.
  색을 바꾸려면 SVG 파일을 직접 수정해야 한다.

## 데모 로그인 (2026-08-07)

**⚠️ 실제 인증이 아니다. 화면 흐름을 맞추기 위한 임시 장치다.**

- 계정은 `js/auth.js` 맨 위 `DEMO_EMAIL` / `DEMO_PASSWORD` 두 상수에 있다.
  현재 값: `futureyouth@gmail.com` / `youth1234`. 바꾸려면 이 두 줄만 수정.
- 로그인 성공 시 `js/session.js`가 sessionStorage에 이메일을 적고 research.html로 이동한다.
  탭을 닫으면 상태가 사라진다(localStorage가 아닌 sessionStorage를 쓰는 이유).
- `js/research.js`가 `<head>`에서 동기 실행되어, 로그인 안 된 접근은 화면이 그려지기 전에
  login.html로 `location.replace` 한다. 뒤로가기로 되돌아오지 않게 replace를 쓴다.
- **한계**: 비밀번호가 소스에 그대로 노출되고, 개발자 도구로 sessionStorage 값을 넣으면
  그대로 통과한다. 실제 회원 데이터를 다루게 되면 서버 세션이나
  인증 서비스(Firebase Auth, Supabase Auth 등)로 통째로 교체해야 한다.
  교체 지점은 `js/auth.js`의 `handleSignIn()`과 `js/session.js` 두 곳뿐이다.
- 이메일 비교는 앞뒤 공백을 자르고 소문자로 맞춘 뒤 대조한다(비밀번호는 대소문자 구분).

## 헤더 구현 규칙 (2026-08-07)

- `.global_nav`는 **`<header>` 안 `.header_inner`의 자식**이다.
  768px 이상에서 로고·메뉴·로그인을 겹침 없이 3분할하려면 같은 그리드에 있어야 한다.
  `.header_inner`는 `grid-template-columns: 1fr auto 1fr`이라 가운데 메뉴가 화면 정중앙에 온다.
- 모바일 드롭다운은 **`position: absolute` (fixed 아님)** 로 헤더 패딩 박스에 붙인다.
  `.header.is_scrolled`의 `backdrop-filter`가 `position: fixed`의 컨테이닝 블록이 되어
  스크롤 전후로 기준이 바뀌기 때문이다. absolute면 영향을 받지 않는다
  (2026-08-07 스크롤 전/후 top·left·width 동일 확인).
- hover 효과는 `@media (min-width: 768px) and (hover: hover)`로 분리한다.
  터치 기기에서 hover가 눌린 채 남는 것을 막고, 키보드는 `:focus-visible`이 담당한다.
- 밝은 배경 페이지(login.html, signup.html)는 `.header_solid`를 함께 붙여 헤더를 불투명하게 만든다.
  기본 헤더 배경은 어두운 히어로 위를 전제로 한 반투명 스크림이라 흰 글자가 묻힌다.
- **클래스 이름 충돌 주의**: `.field`는 이미 메인페이지 '연구 분야' 섹션이 쓰고 있다
  (`layout.css`, `padding: 56px 23px 0`). 로그인 폼은 `.auth_field*`로 접두사를 붙였다.
  새 컴포넌트를 만들 때 기존 클래스명을 먼저 grep할 것.

## 애니메이션 구현 규칙

- 상태 class는 is_open, is_visible 사용 (프로젝트 snake_case 규칙 준수)
- 애니메이션 스타일은 css/animations.css, 인트로는 css/splash.css로 분리
- 인트로 심볼은 파트별 회전이 필요해 index.html에 인라인 SVG로 삽입
  (외부 img로는 내부 path를 CSS로 제어할 수 없음)
- 반짝임(shine)은 SVG 내부 rect에 clipPath(#splash_symbol_clip)를 적용해
  심볼 실루엣 안에서만 보이게 한다. 사각형 div로 덮으면 네모가 빛나 보인다.
- 광원(glow)에 **SVG <g> + filter: drop-shadow()를 쓰면 안 된다.**
  SVG 필터 영역(기본 bbox의 120%)에 블러가 잘려 회전 중 사각형 경계가 드러난다.
  (2026-08-06 브라우저에서 재현·확인) 대신 .splash_glow 원형 radial-gradient
  레이어를 심볼 뒤에 깔아 사용한다. .splash_stage에 overflow:hidden을 주면
  이 광원이 잘리므로 주의.

## 브랜드명 표기

- 정식 명칭은 **미래청년비전연구소** (2026-08-07 사용자 확정)
  - 2026-08-06에 '연구회'로 확정했다가 2026-08-07에 '연구소'로 되돌림.
    표기를 바꿀 때는 index.html / brand/index.html 전체를 함께 치환해야 한다.
- 로고는 심볼만 assets/icons/logo_symbol.svg로 분리해 쓰고,
  글자는 Pretendard 텍스트로 렌더링한다 (헤더 17px / 푸터 19px / 인트로 26px, 700).
  반응형 폭에 따라 글자 크기를 조절해야 해서 SVG 통짜 로고를 쓰지 않는다.
- logo_header.svg, logo_footer.svg(Figma 원본, '연구소' 벡터 텍스트 포함)는
  현재 사용하지 않는다 — 참고용으로만 남겨둠
- 본문 카피의 '미래청년교육원'은 Figma 원문 그대로 유지 중 — 확인 필요
- 스플릿 텍스트 대상은 HTML에 .split_text 클래스로 표시,
  페이지 로드 시 재생할 대상은 data-split-load 속성 추가
- 스크롤 등장 대상은 HTML에 data-reveal 속성으로 표시,
  Stagger 순번은 JS가 같은 부모 내 순서로 --reveal_index에 자동 주입
- 스플릿 시 원문은 부모 요소 aria-label로 보존하고 분해된 글자는 aria-hidden 처리

## 디자인과 다르게 구현한 부분

- Figma 원본의 "연구 분야"는 회색 박스 3개이며 2·3번째 문구가 중복 상태였음.
  사용자 요청에 따라 4개 카드로 확장하고 문구를 사용자 제공안으로 교체함.
- 회색 단색 배경이 더미처럼 보이는 문제를 보완하기 위해 카드에 그라데이션 배경,
  상단 브랜드 액센트 라인, 번호 인덱스(01~04), 카테고리 아이콘을 추가함.

## Our Core Values 이미지 매핑

2026-08-07 사용자가 Figma 원본 사진 4종을 assets/images에 추가해 교체 완료.

| 카드 | 파일 |
|---|---|
| Self Awareness / 자기이해 | `unsplash_zdSoe8za6Hs.png` (청년 실루엣) |
| Metacognition / 메타인지 | `unsplash_6jYoil2GhVk.png` (노트북 작업) |
| Growth / 성장 | `unsplash_qQT7l54ERZM.png` (도시 야경) |
| Connection / 연결 | `unsplash_nzuSrCfX7Eo.png` (회의·협업) |

- 4장 모두 세로 비율 PNG (735~1200px 폭). 카드는 object-fit: cover라 비율 영향 없음.
- 파일당 약 1MB, 합계 4.4MB. 필요 시 JPEG/WebP 변환 및 loading="lazy" 검토.

## 사용 중인 라이브러리

- Pretendard (CDN, jsdelivr): 본문 및 제목 폰트

## 저장 데이터

- 없음 (localStorage 미사용)

## 알려진 문제

- Core Values 이미지 4장 합계 4.4MB (PNG). 최적화 미적용
- 로그인이 데모 고정 계정 방식이라 실제 보안이 없다 (위 '데모 로그인' 참고)
- signup.html은 화면 틀만 있고 실제 가입 처리가 없다. 제출 시 안내 메시지만 노출
- research.html 본문·첨부 자료는 예시 내용이며 다운로드가 연결되어 있지 않다
- program.html 컨설팅 신청서는 접수 서버가 없어 실제로 전송되지 않는다
- program.html 히어로 배경은 Figma 원본(악수하는 4명) 에셋이 없어
  `unsplash_nzuSrCfX7Eo.png`(회의·협업)로 대체했다
- 비밀번호 찾기(`#find_password`)는 앵커 placeholder

## 다음 작업

1. 인증 API 연동 (login.html / signup.html 제출 처리, 로그인 상태에 따른 헤더 전환)
2. 컨설팅 신청서 접수 연동 (program.html)
3. 남은 메뉴 카드·링크의 실제 이동 경로 연결
   (연구소 소개 → brand/index.html, 강의 및 컨설팅 신청 → program.html 연결 완료.
   연구 자료·교류 기관은 아직 앵커 placeholder)
4. 하위 페이지(연구 자료 목록, 교류 기관) 구현
4. Core Values 이미지 경량화 (JPEG/WebP 변환, loading="lazy")

## 마지막 검증 결과 (2026-08-07, 강의 및 컨설팅 절차 페이지)

- 실행 명령: node 정적 서버 (http://localhost:4173)
- 확인 화면: 360 / 768 / 1280px × 메인·브랜드·절차·로그인·회원가입 5개 페이지
- 통과한 항목
  - 15개 조합 전부 가로 스크롤 없음(docW=clientW), 넘치는 요소 0개,
    깨진 이미지 0건, 콘솔 오류 0건, GNB의 program.html 링크 존재
  - 타임라인 세로선이 아이콘 원 정중앙과 정확히 일치 (28/28px, 32/32px)
  - 세부 메뉴 탭이 헤더 높이에 붙어 sticky 동작 (58 / 68 / 76px)
  - 탭 3개가 줄바꿈 없이 한 줄 (높이 42~46px), 신청서 2열 전환은 768px부터
  - 탭 전환: 클릭 시 aria-selected·패널 표시 갱신 + 주소에 `#tab_*` 기록
  - 키보드: ArrowRight로 다음 탭 이동 및 포커스 이동 확인
  - 딥링크: `program.html#tab_apply` 진입 시 컨설팅 신청 탭이 열림
  - JS 차단(sandbox) 상태에서 세 패널 모두 노출, 스텝 10개 전부 읽힘
  - 신청서 검증: 신청 유형(select) → "선택해 주세요", 이름·이메일 → "확인해 주세요",
    개인정보 동의(checkbox) → "체크해 주세요"로 각각 안내하고 해당 항목에 포커스.
    전부 채우면 안내 문구만 노출하고 페이지 이동 없음
  - 신청서 입력 전부 label과 연결됨 (미연결 0건)
- 확인하지 못한 부분
  - **미리보기 창 렌더링이 90×196px로 고장나 스크린샷 검증을 하지 못했다.**
    히어로·탭·1단계까지만 작게 확인했고, 나머지는 DOM 측정값으로만 검증함.
    다음 세션에서 실제 화면을 눈으로 확인할 것
  - 실기기 테스트

## 이전 검증 결과 (2026-08-07, 데모 로그인 + 연구 자료 상세)

- 실행 명령: node 정적 서버 (http://localhost:4173)
- 확인 화면: 360 / 768 / 1280px × 메인·브랜드·로그인·회원가입·연구자료 5개 페이지
- 통과한 항목
  - 15개 조합 전부 가로 스크롤 없음(docW=clientW), 넘치는 요소 0개,
    깨진 이미지 0건, 콘솔 오류 0건
  - 비로그인 상태로 research.html 직접 접근 → 360/768/1280px 모두 login.html로 리다이렉트
  - 틀린 비밀번호 → "이메일 또는 비밀번호가 올바르지 않습니다." + 비밀번호 입력란 포커스,
    세션 저장 안 됨
  - 올바른 계정(앞뒤 공백 + 대문자 섞어 입력) → research.html 이동,
    sessionStorage에 소문자 이메일 저장, 헤더에 이메일 표시
  - 로그아웃 → 세션 삭제 후 login.html로 이동
  - research.html 반응형: 본문 폭 314 / 688 / 760px, 제목 26 / 34 / 40px,
    헤더 이메일은 360px에서 숨김·768px부터 노출, 로그아웃 버튼은 전 폭 노출
  - 스크롤 등장 모션 5개 모두 정상 발동
- 확인하지 못한 부분
  - 미리보기 창 렌더링이 중간에 멈춰, 연구 자료 페이지의 '주요 결과' 이후 구간은
    스크린샷 없이 DOM 측정값으로만 검증함
  - 창 크기 실시간 변경 시의 GNB 모드 전환(matchMedia change)은 이 환경에서
    이벤트가 발생하지 않아 미검증. 각 폭에서 새로고침 시에는 정상
  - 실기기 테스트

## 이전 검증 결과 (2026-08-07, GNB 개편 + 로그인 페이지)

- 실행 명령: node 정적 서버 (http://localhost:4173)
- 확인 화면: 360 / 375 / 768 / 1024 / 1280 / 1440px × 메인·브랜드·로그인 3개 페이지
- 통과한 항목
  - 전 조합에서 가로 스크롤 없음(docW=clientW), 넘치는 요소 0개, 깨진 이미지 0건,
    콘솔 오류 0건
  - 모바일 헤더: 로고(23~178) / 계정 아이콘(284~313) / 햄버거(325~354) 겹침 없음
  - 모바일 드롭다운: 스크롤 전후 모두 top=58 / left=0 / width=375로 동일
    (backdrop-filter 컨테이닝 블록 전환에도 위치 고정 확인)
  - 768px: 로고 심볼만(40~61), GNB 172~556(화면 중앙 대비 -12px),
    로그인·회원가입 572~713, 햄버거·계정 아이콘 display:none
  - 1024 / 1280 / 1440px: 로고 글자 복귀, GNB 화면 정중앙(offset 0),
    로고↔메뉴 / 메뉴↔로그인 간격 156~202px, 히트 테스트 3영역 모두 정상
  - hover(실제 마우스 이동으로 확인)
    - 메뉴 항목: 색 rgba(255,255,255,0.82) → #fff, 밑줄 scaleX 0 → 1
    - 로그인: 배경 rgba(255,255,255,0.16) 차오름
    - 회원가입: 브랜드 그라데이션 채움 + 그림자 + translateY(-1px)
    - 현재 페이지(aria-current) 항목은 hover 없이도 밑줄 유지
  - 로그인 폼: 빈 값·잘못된 이메일 → has_error 메시지 + 해당 입력 포커스,
    정상 입력 → 안내 메시지, 어느 경우에도 페이지 이동 없음
  - 모든 input이 label 또는 aria-label과 연결됨 (미연결 0건)
- 확인하지 못한 부분
  - 창 크기를 실시간으로 바꿀 때의 GNB 모드 전환(matchMedia change)은
    이 환경에서 change 이벤트가 발생하지 않아 미검증. 각 폭에서 새로고침 시에는 정상
  - 실기기 테스트, 터치 기기에서의 hover 잔상 (CSS로만 방어)

## 이전 검증 결과 (2026-08-07, 반응형 + 브랜드 페이지)

- 실행 명령: node 정적 서버 (http://localhost:4173)
- 확인 화면: 360px, 375px, 768px, 820px, 1280px (메인 / 브랜드 소개 두 페이지)
- 통과한 항목
  - 두 페이지·전 브레이크포인트에서 가로 스크롤 없음(docW=clientW), 넘치는 요소 0개
  - 콘솔 오류 0건, 리소스 전체 200 OK, 깨진 이미지 0건
  - 토큰 전환: --page_padding 23 → 40 → 48px, --header_height 58 → 68 → 76px
  - 메인 menu_grid 2열(360·820px) → 4열(1280px), field_list 1열 → 2열 → 4열,
    link_list·footer_inner 1열 → 2열
  - 브랜드 value_list 1열 → 2열(327px) → 4열(277px),
    history_track 데스크톱에서 900px 폭 중앙 정렬
  - 데스크톱 GNB: 햄버거 display:none, 전체 메뉴가 헤더에 가로 배치(우측 정렬),
    로고·nav 링크 모두 elementFromPoint 히트 테스트 통과(pointer-events 충돌 없음)
  - 링크: 메인 GNB '연구소 소개'·소개 카드 → brand/index.html,
    브랜드 로고 → index.html, 브랜드 GNB 나머지 → ../index.html#앵커
  - 375px·768px 스크린샷으로 히어로 / Vision·Mission / Core Values / History 시각 확인
- 확인하지 못한 부분
  - 데스크톱(1280px) 스크린샷은 히어로·GNB만 확보. 이후 미리보기 창이 hidden 상태가 되어
    나머지 섹션은 DOM 측정값(위치·크기·컬럼 수)으로만 검증함
  - 미리보기 창이 hidden이면 IntersectionObserver·requestAnimationFrame이 멈추므로
    해당 상태에서의 스크롤 등장 모션·메뉴 토글 실동작은 재현 불가.
    375px·768px 가시 상태에서는 정상 동작 확인함
  - 창 크기를 실시간으로 바꿀 때의 GNB 모드 전환(matchMedia change)은
    이 환경에서 change 이벤트가 발생하지 않아 미검증. 각 폭에서 새로고침 시에는 정상
  - 실기기 테스트

## 컨설팅 신청 흐름 (2026-08-07)

- Figma 7개 시안 구현: 컨설팅 절차 / 신청자 정보 / 희망 일정 / 약관 동의 /
  개인정보 수집·이용 동의 / 개인정보 제3자 제공 동의 / 신청 완료
- 완료한 내용
  - program.html 컨설팅 신청 탭: 인라인 신청서를 3단계 안내 + `신청하기` 버튼으로 교체.
    버튼은 로그인 버튼(.auth_submit)과 같은 그라데이션 필 디자인, apply_step1.html로 이동
  - 새 페이지 6개: apply_step1(신청자 정보) → apply_step2(희망 일정) →
    apply_step3(약관 동의) → apply_done(완료).
    apply_step3에서 apply_terms_privacy / apply_terms_third_party 상세로 분기 후 복귀
  - 새 파일: css/apply.css, js/apply.js
  - 약관 전문은 Figma 텍스트 노드 원문을 그대로 사용(요약·재작성 없음)
  - 입력값은 sessionStorage("youth_apply_draft")에만 저장하고 완료 화면에서 비운다
  - 달력은 실제 동작(월 이동, 다중 선택, 초기화)으로 구현
- 주요 판단
  - 시안의 단색 버튼(#29a1ff, radius 38) 대신 로그인 버튼과 동일한 그라데이션 필 사용.
    사용자가 "로그인 버튼과 동일하게"를 명시했으므로 시안보다 우선 적용
  - 시안 헤더는 반투명 검정이나 배경이 밝아 가독성이 없어 login.html과 같은
    header_solid를 사용
  - program.html의 기존 인라인 신청서는 새 단계 흐름으로 대체되어 제거.
    이에 따라 program.css의 폼 전용 스타일과 js/program.js의 폼 검증 코드도 함께 제거
  - 컨설팅 절차 시안(41:2787)은 program.html 컨설팅 절차 탭에 이미 구현되어 있어
    그대로 두었다. 시안에는 "프로그램 설계" 단계가 두 번 반복되지만
    기존 5단계 구성이 더 정확하다고 보고 유지함
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 전체 흐름 통과: step1 입력 → step2 날짜 3개 선택 → step3 전체 동의 → apply_done
  - 약관 상세에서 동의 후 확인 → step3로 복귀하며 해당 항목만 체크된 상태 유지
  - 미동의 상태로 제출 시 "필수 약관에 모두 동의해 주세요." 안내 후 이동 차단
  - 360px / 375px / 1280px 확인, 가로 스크롤 없음, 콘솔 오류 0건
  - 모든 input에 연결된 label 존재, 내부 링크 전부 실제 파일로 연결됨
- 고친 문제
  - 새 페이지에 #to_top 버튼이 없어 js/main.js가 addEventListener에서 예외를 던짐 → 6개 전 페이지에 추가
  - 체크박스를 자기 자신을 가리키는 label이 감싸고 있어 활성화가 두 번 토글되던 문제 →
    input을 label 밖으로 빼고 체크 표시와 텍스트를 각각 label[for]로 변경
- 확인하지 못한 부분
  - 키보드 Space로 체크박스를 토글하는 동작. 이 환경의 key 입력이 페이지에 전달되지 않아
    기존 login.html 체크박스에서도 동일하게 반응 없음(도구 한계). 클릭 경로는 정상 확인
  - 실기기 테스트, 실제 접수 서버 연동

## 컨설팅 절차 시안 반영 · 내비게이션 정리 (2026-08-07)

- Figma 시안: 47:2931 (컨설팅 절차 상세 수정본)
- 완료한 내용
  - program.html 히어로 우측 아이콘 묶음(.program_hero_icons) 삭제 — 새 시안에 없음
  - 컨설팅 절차 5단계 → 시안대로 6단계로 교체.
    신청서 작성 / 사전 검사지 작성 / 프로그램 설계 / 일정 확정 / 교육 진행 / 사후 피드백
  - 탭(강의 절차 / 컨설팅 절차 / 컨설팅 신청)을 누르면 해당 패널 h2까지 스크롤.
    sticky 헤더 + 탭 바 높이를 빼서 제목이 가리지 않게 하고, prefers-reduced-motion이면 즉시 이동
  - "이 신청서는 화면 틀입니다…" 안내 문구 삭제 (.apply_notice 마크업·스타일 모두)
  - 연구 자료 / 교류 기관(네트워크) / 연구 분야 내비 링크 → 전 페이지에서 login.html
    (brand/index.html은 ../login.html). 12개 HTML 일괄 적용
  - index.html menu_section 카드 4장 모두 실제 페이지로 연결:
    연구소 소개 → brand/index.html, 연구 자료 → login.html,
    교류 네트워크 → login.html, 강의 및 컨설팅 신청 → program.html
- 주요 판단
  - 시안의 컨설팅 절차에는 STEP 라벨이 없어 해당 탭에서만 제거(.step_item_plain).
    강의 절차 탭은 손대지 않아 STEP 01~05가 그대로 남아 있다
  - 아이콘은 시안 원본을 새로 받지 않고 기존 파란 선 아이콘 세트를 재사용.
    시안 그림에 맞춰 프로그램 설계 → icon_step_present(화이트보드),
    교육 진행 → icon_step_curriculum으로 매핑 변경
  - research.html의 '연구 자료' 내비에 있던 aria-current="page"는 링크가 login.html로
    바뀌면서 함께 제거됨
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 탭 클릭 후 h2가 탭 바 바로 아래 16px 지점에 위치 (h2Top 149 / tabsBarBottom 133)
  - 히어로 아이콘 0개, .apply_notice 0개, 컨설팅 절차 .step_index 0개 확인
  - 12개 HTML 전체에서 연구 자료·교류 기관·연구 분야 내비가 모두 login.html로 연결됨
  - 메인 '연구 자료' 카드 클릭 → login.html 이동 확인
  - 375px 확인, 가로 스크롤 없음, 콘솔 오류 0건
- 확인하지 못한 부분
  - 컨설팅 절차 5·6번째 단계는 data-reveal 때문에 처음에는 opacity 0이고
    스크롤해야 나타난다. 기존 등장 모션 규칙 그대로이며 의도된 동작으로 두었다
  - 실기기 테스트

## 데이터 아카이브 신설 · 연혁 교체 · 신청 폼 개선 (2026-08-07)

- 완료한 내용
  - brand/index.html 연혁을 새 14건으로 전면 교체(최신순 2026.08 → 2019.03).
    마지막 항목은 「미래청년비전연구소 창립」. 새 내용에 설명줄이 없어
    history_desc 마크업과 css/brand.css의 관련 규칙을 함께 제거
  - 전 페이지(13개) 내비에 「데이터 아카이브」 추가 → archive.html
    (brand/index.html은 ../archive.html). '강의 및 컨설팅 신청' 다음 자리
  - archive.html 신규: 청년 성장 데이터 아카이브.
    지표 카드 5종(누적 연구 대상자 3,582명 / 누적 멘토링 18,430회 /
    연구 프로그램 42개 / 성장 데이터 253,000건 / 연구 보고서 37건),
    연도별 조사 데이터 탭 8개(2026 → 2019, 최신순)
  - 새 파일: css/archive.css, js/archive.js
  - apply_step1.html 생년월일: type="date" 입력 → 연/월/일 select 3개.
    윤년·월별 일수를 실제 달력대로 계산해 일 목록을 다시 채우고,
    사라진 날짜(2월 30일 등)를 고른 상태였으면 비운다
  - 달력 주말 글자색을 평일과 동일하게 통일(css/apply.css .apply_date_weekend).
    선택 불가 날짜의 disabled 스타일은 그대로 유지
- 주요 판단
  - 연도별 조사 데이터는 연혁에 실제로 있는 항목만 옮겼다. 없는 연도용
    「연구 데이터 준비 중」 표시는 js/archive.js에 넣어두었으나,
    2019~2026 8개 연도 모두 연혁 항목이 있어 현재는 나타나지 않는다
  - 참고 이미지의 회색 카드/남색 탭 대신 기존 브랜드 색(--color_brand 그라데이션)과
    기존 탭 스타일(program.css의 .program_tab과 같은 규칙)을 따랐다
  - archive.js의 탭 동작은 program.js와 같은 규칙(선택 탭만 Tab 순서, 좌우 화살표 이동)
  - 「기관」 입력 항목은 이미 존재하지 않았다. 이전 세션에서 program.html 인라인
    신청서를 제거할 때 '소속(organization)' 필드가 함께 사라졌고,
    남은 '교류 기관'은 내비 메뉴 이름이라 건드리지 않았다
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 13개 페이지 전수 검사: 깨진 내부 링크 0, 없는 CSS/JS/이미지 0,
    데이터 아카이브 내비 누락 0, 기관/소속 입력 항목 0
  - 연혁 14건, 최신순 정렬, 마지막 「미래청년비전연구소 창립」 확인
  - 연도 탭 2019/2023/2025 클릭 시 해당 패널만 열림 확인
  - 윤년: 2024.02=29일, 2023.02=28일, 2000.02=29일,
    4·6·9·11월=30일, 1·3·5·7·8·10·12월=31일
  - 2024.02.29 선택 후 2023년으로 바꾸면 일 선택이 비워짐,
    3월 15일 → 5월로 바꾸면 15일 유지
  - 1단계 제출 시 draft.birth가 "2001-02-28" 형식으로 저장되고 2단계로 이동
  - 달력 토/일과 평일 글자색 모두 rgb(16,16,16)로 동일
  - 1280px: 지표 카드 5개 한 줄 / 375px: 2열 3행, 연도 탭 가로 스크롤(642px > 329px)
  - 가로 스크롤 없음, 텍스트 잘림 0, 콘솔 오류 0건
  - 기존 기능 유지 확인: 탭 클릭 시 h2 스크롤, 컨설팅 절차 6단계
- 확인하지 못한 부분
  - 지표 5종 수치는 사용자가 제공한 예시 데이터이며 실제 집계값이 아니다
  - 참고 이미지가 대화에 전달되지 않은 상태에서 글 명세로 먼저 제작했고,
    이후 전달된 이미지로 구성(지표 카드 그리드, 연도 탭 + 그룹형 목록)만 맞췄다
  - 실기기 테스트

## 연구 성과 섹션 · 로그인 게이트 (2026-08-08)

- Figma 시안: 49:3069(메인 + 연구 성과), 49:3274(신청 완료 + 연구원 코드)
- 완료한 내용
  - index.html에 「연구 성과」(지표 3종)와 「최신 연구보고서」(가로 스크롤 카드) 섹션 추가.
    시안 순서대로 메뉴 카드 다음에 넣고, 기존 about / field / link_section은 그대로 둠
  - 49:3069의 하단 배너(R&D 업무포털 / 국가 연구자 정보 시스템)는 이미 index.html에
    .link_list로 구현되어 있어 새로 만들지 않고 그대로 두었다
  - 지표 카드(.stat_*)를 archive.css → components.css로 옮겨 아카이브와 공용.
    archive.html은 이미 components.css를 불러오므로 HTML 변경 없음
  - 신청하기 로그인 게이트: 링크에 data-requires_auth를 붙이면 session.js가 가로채
    비로그인 시 원래 주소를 sessionStorage에 적고 login.html로 보낸다
  - auth.js: 로그인 성공 시 takeRedirect()로 원래 주소 복귀, 없으면 기존대로 research.html.
    이미 로그인한 사람이 로그인 화면에 오면 곧장 목적지로 replace
  - apply_done.html에 연구원 코드 칩 추가(시안 49:3274)
- 주요 판단
  - 요구사항 6번은 "로그인 성공 → 49:3274 화면"이지만, 4번은 "원래 신청하려던 페이지로 복귀"다.
    두 요구가 충돌해 4번(표준 패턴)을 따랐다. 로그인 후 apply_step1로 복귀하고
    신청 절차 끝에서 49:3274(apply_done)를 만난다.
    로그인 직후 완료 화면으로 보내려면 js/auth.js의 SIGNED_IN_REDIRECT 계열 분기만 바꾸면 된다
  - 로그인 상태 저장은 기존 sessionStorage(youth_signed_in_email) 그대로 재사용.
    새 키는 youth_redirect_after_login 하나뿐이고 비밀번호는 저장하지 않는다
  - takeRedirect()는 절대 URL/프로토콜을 걸러 같은 사이트 상대 경로만 받아들인다
  - 연구원 코드는 시안 값(93857291EA)을 그대로 표시. 실제 발급은 서버 몫이라
    난수 생성기를 만들지 않았다
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 비로그인 신청하기 → login.html 이동, youth_redirect_after_login="apply_step1.html",
    "신청을 이어가려면 먼저 로그인해 주세요." 안내 표시
  - 로그인 성공 → apply_step1.html 복귀, 저장된 주소 삭제됨
  - 로그인 상태에서 신청하기 재클릭 → 로그인 페이지 거치지 않고 바로 apply_step1.html
  - 새로고침 후 isSignedIn() true 유지, 저장 키는 youth_signed_in_email 하나뿐
  - 게이트 없이 로그인하면 기존대로 research.html (기존 동작 유지)
  - 1280px: 지표 3열 한 줄 / 375px: 2열, 보고서 카드 가로 스크롤
  - 13개 페이지 깨진 링크 0, 없는 에셋 0, 콘솔 오류 0, 가로 스크롤 0, 텍스트 잘림 0
- 확인하지 못한 부분
  - 지표 수치(1,222 / 1,109 / 613)와 보고서 카드 3건은 시안의 예시 데이터다.
    보고서 표지는 기존 사이트 이미지를 재사용했고 모두 research.html로 연결된다
  - 로그인해도 헤더는 계속 '로그인 / 회원가입'을 보여준다(로그아웃 전환 미구현)
  - 데모 계정(futureyouth@gmail.com / youth1234)은 auth.js에 그대로 있어 실제 인증이 아니다
  - 실기기 테스트

## 메인페이지 시안 재반영 (2026-08-08)

- Figma 시안: 49:3069(수정본)
- 완료한 내용
  - 섹션 순서를 시안대로 정정: 메뉴 카드 → about 밴드 → 연구 성과 → 최신 연구보고서 → 연구 분야
    (직전에는 연구 성과/보고서가 about 앞에 있었다)
  - about 밴드 배경을 Figma 원본 이미지로 교체. 본문도 시안 문구
    "미래청년교육원은 … 비영리 교육 연구 기관입니다"로 맞춤
  - 연구 성과 카드를 시안대로 파란 그라데이션(#bee6ff→#68c5ff, radius 12) +
    흰색 반투명 원(47px) 아이콘 + 좌측 정렬 라벨/수치로 변경
  - 최신 연구보고서를 사진 카드 → 실제 연구보고서 표지로 재구현
    (코드 칩 / 제목 / 저자 / 네트워크 일러스트 / 안내 문구 / 로고)
  - 데스크톱에서 표지 3장이 콘텐츠 폭을 가득 채우도록 768px부터 3열 grid로 전환.
    모바일은 기존대로 가로 스크롤 유지
- 사용자가 넣어준 에셋 (assets/images/)
  - "Mask group2.png" → about_band.png 로 이름 변경. Figma 노드 51:3559/51:3552와 일치(429x244)
  - "Mask group.png" → report_cover_graphic.png 로 이름 변경.
    Figma 노드 51:3551(428x659)이며 연구보고서 표지의 네트워크 일러스트다.
    파일 전체를 훑어 이름이 Mask group인 노드 11개의 크기를 대조해 확정했다
  - 이 PNG는 위쪽이 흰 여백인 불투명 이미지라, 래퍼로 아래쪽 46%만 잘라 표지 하단에 깐다.
    표지 글자에는 z-index 1을 줘 일러스트 위로 올렸다
- 주요 판단
  - 연구 성과 카드는 데이터 아카이브와 공용인 .stat_* 기본형은 그대로 두고
    .result 안에서만 그라데이션으로 덮어썼다. 아카이브 카드는 영향 없음(검증함)
  - 표지 안쪽 글자 크기는 cqw(컨테이너 쿼리 단위)로 잡아 카드 폭이 달라져도 비율이 유지된다
  - 시안에 없는 3번째 표지는 research.html의 실제 글(메타인지 기반 청년 교육 프로그램 효과 분석)을
    쓰고 저자명은 지어내지 않았다
  - 시안의 연구 분야가 통이미지(image 35/36)로 바뀌었지만, 기존 HTML 카드 구현이
    더 낫다고 보고 그대로 두었다
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 1280px: 표지 3장이 각 379px, 한 줄, 콘텐츠 폭 1184px를 정확히 채움, 가로 스크롤 없음
  - 375px: 지표 3개 한 줄(각 102px), 표지는 가로 스크롤, 텍스트 잘림 0
  - 표지 글자가 일러스트에 가리지 않음(제목/저자/안내문구/로고 모두 노출)
  - 아카이브 지표 카드 원래 스타일 유지(흰 배경 #f7fbff, 가운데 정렬, 1px 테두리)
  - 13개 페이지 깨진 링크 0, 없는 에셋 0, 콘솔 오류 0
- 확인하지 못한 부분
  - 표지 코드(연구보고25-수시05/06)와 저자명은 시안에 적힌 값이며 실제 발간물이 아니다
  - 실기기 테스트

## 메인페이지 여백 · 연구 성과 재설계 · 숫자 카운팅 (2026-08-08)

- 완료한 내용
  - 섹션 리듬을 --section_space 토큰 하나로 통일(base.css).
    모바일 72px / 768px 96px / 1024px 128px.
    각 breakpoint에 흩어져 있던 .intro·.menu_section·.field·.link_section·.about의
    padding-top 개별 지정을 지우고 토큰이 몰아서 결정하게 했다
  - 풀블리드 밴드(.about, .report)는 안쪽 여백만 따로 키운다(56 → 76 → 104px)
  - 연구 성과 카드 재설계: 파란 그라데이션 → 흰 카드 + 1px 테두리 +
    상단 3px 브랜드 그라데이션 라인 + 브랜드 톤 아이콘 원.
    내용(정기간행물/학술행사자료/영상, 수치)은 그대로
  - 수치는 화면에 들어올 때 1부터 목표값까지 올라간다(data-count_to).
    js/main.js의 기존 showElement에 얹어 IntersectionObserver를 새로 만들지 않았다.
    easeOutCubic 1.4초, toLocaleString("ko-KR")로 천 단위 구분,
    prefers-reduced-motion이면 최종값을 바로 표시, data-count_done으로 1회만 실행
  - 숫자 폭이 출렁이지 않도록 .result .stat_value에 font-variant-numeric: tabular-nums
  - 모바일 최신 연구보고서의 가로 스크롤 막대 숨김
    (scrollbar-width: none / -ms-overflow-style / ::-webkit-scrollbar).
    파란 띠 위에 회색 막대가 걸쳐 보이던 문제. 스와이프·키보드 스크롤은 유지
- 주요 판단
  - "가로 스크롤 바 삭제"는 막대만 감추고 스와이프는 남기는 쪽으로 읽었다.
    3장을 모바일에서 세로로 쌓으면 표지가 과하게 커지고 섹션이 길어진다
  - 연구 성과 카드는 여전히 .result 안에서만 덮어쓰므로 아카이브 지표 카드는 그대로다
  - 여백 토큰을 base.css에 뒀지만 .intro/.about/.field/.link_section/.result/.report는
    index.html에서만 쓰여 다른 페이지에는 영향이 없다(전수 확인함)
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 1024px+: 섹션 간격이 모두 128px로 균일(about→연구성과, 연구성과→보고서,
    보고서→연구분야 모두 128px)
  - 768px: 토큰 96px, 보고서 3장 한 줄(각 213px), 스크롤 막대 0px
  - 375px: 토큰 72px, 스크롤 막대 0px, 스와이프 가능, 가로 스크롤 없음
  - 카운팅: 진입 전 최종값 → 진입 250ms 시점 485/440/243 → 1.6초 후 1,222/1,109/613
  - 콘솔 오류 0건
- 확인하지 못한 부분
  - prefers-reduced-motion 실제 OS 설정에서의 동작은 코드 경로만 확인했다
  - 실기기 테스트

## 연구 성과 글래스 카드 · 보고서 모바일 세로 배치 (2026-08-08)

- 참고: thepureglobal.com 사업 영역 섹션(사용자 첨부 이미지)
- 완료한 내용
  - 연구 성과를 레퍼런스 구조로 재구성:
    왼쪽 = 점 + "RESEARCH OUTPUT" eyebrow + 큰 제목 + 설명,
    오른쪽 = 01/02/03 번호가 붙은 카드 3장
  - 카드를 글래스로: 흰색 반투명 그라데이션 + backdrop-filter blur(18px) +
    흰 테두리 + 안쪽 하이라이트 + 파란 그림자, radius 24px.
    카드 안 오른쪽 위에서 번지는 원형 광원(::after) 추가.
    backdrop-filter는 @supports로 감싸 미지원 브라우저는 반투명 배경만 보인다
  - 글래스가 비칠 바탕이 필요해 .result 섹션에 옅은 파란 그라데이션 배경을 깔았다
  - 3D 아이콘 3종 신규 제작(SVG, 직접 작성):
    icon_3d_journal(겹친 유리 책), icon_3d_conference(유리 발표보드+막대그래프),
    icon_3d_video(유리 큐브+재생+궤도링). 그라데이션·하이라이트·drop-shadow로 입체감
  - 지표 내용(정기간행물 1,222 / 학술행사자료 1,109 / 영상 613)과
    1부터 올라가는 카운팅은 그대로 유지. 카드마다 짧은 설명 한 줄 추가
  - 모바일 최신 연구보고서를 가로 스크롤 → 세로 3행으로 변경.
    표지가 과하게 커지지 않도록 max-width 260px + 가운데 정렬
- 주요 판단
  - 레퍼런스의 원형 화살표 버튼은 넣지 않았다. 지표 카드는 이동할 곳이 없어
    누를 수 없는 버튼이 되기 때문
  - 좌/우 2단 분할은 1024px이 아니라 1280px부터 적용했다.
    1024px에서는 카드 폭이 172px까지 눌려 아이콘과 숫자가 넘쳤다
  - 3D 아이콘은 레퍼런스 사이트 에셋을 가져오지 않고 SVG로 직접 그렸다
  - 카드 뼈대(.stat_list/.stat_card)는 여전히 .result 안에서만 덮어써
    데이터 아카이브 지표 카드 5장은 그대로다(확인함)
- 고친 문제
  - 탭이 백그라운드면 requestAnimationFrame이 멈춰 숫자가 중간값(86/78/44 등)에
    머무는 것을 확인. COUNT_DURATION + 400ms 타이머로 최종값을 한 번 확정하게 보완했다.
    정상 재생일 때는 이미 목표값이라 변화가 없다
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 1380px: result_inner 300px + 836px 2단, 카드 265px, 숫자 한 줄(44px)
  - 768px: 카드 3장 한 줄, 보고서 3장 한 줄
  - 375px: 연구 성과 세로 3행, 보고서 세로 3행(각 260px),
    가로 스크롤 자체가 없음(scroller.scrollWidth == clientWidth)
  - 카운팅 최종값 1,222 / 1,109 / 613, backdrop-filter blur(18px) 적용됨
  - 13개 페이지 깨진 링크 0, 없는 에셋 0, 콘솔 오류 0, 텍스트 잘림 0
- 확인하지 못한 부분
  - 카드 설명 한 줄("연구 성과를 정기적으로 묶어 펴낸 간행물" 등)은 새로 쓴 문구다
  - 실기기 테스트

## 연구 자료 시스템 확장 (2026-08-08)

- Figma 시안: 55:3567(연구보고서01), 55:3691(02), 55:3815(03)
- 새로 만든 파일
  - research_list.html — 연구 자료 목록
  - research_report.html — 연구보고서 상세 (?id= 로 3개 시안을 모두 처리)
  - js/research_data.js — 연구 자료 단일 데이터 소스
  - js/research_list.js, js/research_report.js
- 수정한 파일
  - index.html: 최신 연구보고서 카드 3장을 각각 research_report.html?id=... 로 연결하고
    data-requires_auth 부여. 메뉴 카드 '연구 자료'도 목록으로. 카드 3의 제목/코드/저자를
    데이터와 맞춤(메타인지 → 자기주도적 성장)
  - 전 페이지(13개) 내비 '연구 자료' → research_list.html + data-requires_auth
  - research.html: '연구 자료 목록' 버튼을 index.html#section_menu → research_list.html
  - js/research.js: 비로그인 시 현재 주소(?id= 포함)를 setRedirect로 저장한 뒤 로그인으로 보냄
  - css/research.css: 목록/상세 스타일 추가 (기존 파일 확장, 새 CSS 파일 만들지 않음)
- 재사용한 기존 구조 (새로 만들지 않은 것)
  - 로그인: js/auth.js 데모 계정 + sessionStorage(youth_signed_in_email) 그대로
  - 게이트: js/session.js의 [data-requires_auth] + youth_redirect_after_login 그대로.
    FLOW B(보고서 → 로그인 → 그 보고서 상세)는 이 기존 장치만으로 동작한다
  - 접근 제어/로그아웃: js/research.js를 새 두 페이지에서 그대로 사용
  - 헤더/푸터/to_top/헤더 사용자 표시는 research.html 마크업을 그대로 복제
- 데이터 구조
  - js/research_data.js가 REPORTS(새 3건) + LEGACY(기존 research.html 1건)를 갖고
    toListItems()로 합쳐 준다. 목록과 상세, 메인 카드가 같은 제목을 바라본다
  - 초록/목차 본문은 Figma 텍스트 노드 원문. Figma가 줄바꿈에 \n과 U+2028을 섞어 써
    둘 다 줄바꿈으로 처리했다(안 하면 초록이 한 덩어리가 된다)
- 고친 문제
  - js/research_list.js가 만든 카드에 data-reveal을 붙였더니 영영 안 보였다.
    main.js는 페이지 최초 1회만 [data-reveal]을 모으므로 나중에 만든 요소는
    관찰 대상이 아니다 → 동적 카드에서는 data-reveal을 빼는 것으로 해결
  - research_report.html 본문(초록/목록/서지)에서도 data-reveal을 뺐다.
    긴 문서가 스크롤 등장에 가려지면 백그라운드 탭·앵커 점프에서 안 읽힌다
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - FLOW A: 내비 연구 자료 → 로그인 → research_list.html, 전체 4건
    (기존 1 + 신규 3), 각 카드가 서로 다른 상세로 연결됨
  - FLOW B: 비로그인 보고서 B 클릭 → 저장값 research_report.html?id=job_readiness →
    로그인 → 그 상세 도착. 보고서 C도 self_directed로 정확히 도착
  - 로그인 상태에서 보고서 A 클릭 → 로그인 페이지 거치지 않고 바로 상세
  - 상세 렌더링: 키워드 10개, 초록 4단락, 목차 28줄, 서지 8줄 (3건 모두 동일 구조)
  - 잘못된 id → "요청하신 연구 자료를 찾을 수 없습니다" + 목록 링크
  - 저장 키는 youth_signed_in_email 하나뿐(비밀번호 미저장)
  - 375px 1열 / 768px 2열 / 1024px 3열, 가로 스크롤 0, 텍스트 잘림 0
  - 15개 페이지 깨진 링크 0, 없는 에셋 0, 콘솔 오류 0
- 확인하지 못한 부분
  - 보고서 표지 이미지는 기존 사이트 사진을 재사용했다(시안 표지 원본 아님)
  - 보고서 원문 PDF 다운로드는 연결되어 있지 않다(화면에도 그렇게 안내)
  - 데모 계정이 js/auth.js에 그대로 있어 실제 인증이 아니다
  - 실기기 테스트

## 안내 문구 제거 · 기관명 변경 (2026-08-08)

- 완료한 내용
  - 페이지에 고정으로 붙어 있던 안내 문구 2곳 삭제
    · research_report.html "본문은 연구보고서 서지 정보와 초록, 목차를 옮긴 화면입니다…"
    · research.html "이 페이지는 연구 자료 상세 화면의 임시 레이아웃입니다…"
    마크업과 함께 css/research.css의 .research_notice 규칙, 빈 구획 주석,
    파일 첫 줄의 "임시 레이아웃" 설명도 정리했다
  - 기관명 "미래청년교육연구소" → "미래청년비전연구소" 일괄 변경 (84건)
    html 16개 + js 2개(research_data.js, research_report.js).
    헤더·푸터 로고 글자, page title, meta description, 본문 모두 포함
- 손대지 않은 것
  - 로그인/회원가입 폼의 data-pending-message
    ("인증 서버가 아직 연결되지 않아 …"). 이건 고정 안내가 아니라
    제출했을 때만 뜨는 폼 응답이라, 지우면 버튼을 눌러도 아무 반응이 없어진다
  - "미래청년비전연구소"(개인정보 처리방침 본문 15건), "미래청년교육원"(2건),
    "미래청년교육 5단계 교육모델" 같은 프로그램·시리즈 이름은 그대로 두었다.
    요청한 문자열이 아니고 기관명이 아닌 고유명사이기 때문
  - futureyouth(영문 표기), futureyouth@gmail.com도 그대로
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 15개 페이지에서 옛 기관명 0건, .research_notice 0건
  - 모든 page title이 "… | 미래청년비전연구소"로 갱신됨
  - FLOW B 회귀 정상: 비로그인 보고서 B 클릭 → 저장값 research_report.html?id=job_readiness
    → 로그인 → 해당 상세 도착, 본문 3블록 모두 노출
  - 연구 자료 목록 4건(기존 1 + 신규 3) 링크 정상, 기존 research.html 본문·첨부 자료 유지
  - 깨진 링크 0, 없는 에셋 0, 콘솔 오류 0, 가로 스크롤 0
- 확인하지 못한 부분
  - 로고 이미지(assets/icons/logo_*.svg)에 글자가 들어 있다면 별도 교체가 필요하다.
    현재 헤더·푸터는 심볼 SVG + HTML 글자 조합이라 이번 변경으로 충분하다
  - 실기기 테스트

## 로그인 문구 정리 · 표기 통일 · 파비콘/OG (2026-08-08)

- 완료한 내용
  - 로그인·회원가입에서 "인증 서버" 표현 삭제
    · login.html: data-pending-message 제거 (로그인은 handleSignIn이 따로 처리해 쓰이지 않던 속성)
    · signup.html: "회원가입이 완료되었습니다. 로그인 후 이용해 주세요."로 교체
    · js/auth.js: DEFAULT_PENDING_MESSAGE → "입력하신 내용이 확인되었습니다.", 주석도 정리
  - 개인정보 처리방침의 "미래청년교육연구회" → "미래청년비전연구소" 15건
    (apply_terms_privacy 14건, apply_terms_third_party 1건)
  - FeatureYouth / featureyouth → futureyouth 44건.
    저작권 표기와 이메일(futureyouth@gmail.com) 모두 포함.
    ⚠️ js/auth.js의 데모 계정 이메일도 함께 바뀌었다 → 이제 futureyouth@gmail.com / youth1234
  - 파비콘·OG 이미지 신규 제작 후 15개 페이지에 메타 삽입
    · assets/icons/favicon.svg (브랜드 그라데이션 라운드 사각형 + 로고 심볼)
    · assets/icons/favicon.png (64px), apple_touch_icon.png (180px)
    · assets/images/og_image.svg (원본), og_image.jpg (1200x630, 85KB)
    · link rel=icon / alternate icon / apple-touch-icon, theme-color,
      og:type·site_name·title·description·image(+width/height)·url, twitter:card
    · 페이지별 title/description을 그대로 og:title/og:description으로 넣어 중복 문구를 만들지 않았다
- 주요 판단
  - 로고 심볼 SVG의 path를 그대로 재사용해 파비콘·OG를 만들었다(새 그림 없음)
  - OG는 SNS가 래스터를 요구해 PNG로 굽고 JPEG로 압축했다(759KB → 85KB).
    맥에 rsvg-convert·ImageMagick·PIL이 없어 qlmanage로 렌더링했는데,
    정사각 캔버스에 맞춰 확대/축소하는 성질이 있어
    1200x1200 래퍼 안에 원본을 얹어 1:1로 렌더링한 뒤 가운데 630px를 잘랐다
  - og:url의 도메인은 https://futureyouth.example 임시값이다.
    배포 주소가 정해지면 15개 페이지의 이 문자열만 바꾸면 된다
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 15개 페이지에서 "인증 서버" / FeatureYouth / featureyouth /
    미래청년교육연구회 / 미래청년교육연구소 모두 0건
  - 15개 페이지 전부 favicon·apple-touch-icon·og:image·twitter:card 메타 보유
  - 파비콘/OG 파일 5종 모두 정상 서빙, og_image.jpg 1200x630
  - 회원가입 제출 → "회원가입이 완료되었습니다…" (서버 언급 없음)
  - 로그인 실패 → "이메일 또는 비밀번호가 올바르지 않습니다."
  - 로그인 성공(futureyouth@gmail.com) → research.html 이동, 세션 유지
  - 없는 에셋 0, 콘솔 오류 0
- 확인하지 못한 부분
  - og:url 실제 도메인 미정
  - SNS 실제 공유 미리보기(카카오·페이스북 디버거)는 배포 후 확인 필요
  - 실기기 테스트

## 헤더 로고 글자 복원 (2026-08-08)

- 문제
  - 768~1023px에서 로고 글자가 안 보였다. 마크업이 지워진 게 아니라
    css/responsive.css에 "가로 폭이 빠듯해 로고 글자를 숨긴다"는 규칙이 있었다.
    메뉴가 5개이던 시절의 결정인데 지금은 6개라 더 빠듯해진 상태였다
- 실측 (768px, 헤더 사용 가능 폭 688px)
  - 로고 글자만 켜면 필요 폭 784px → 96px 초과
  - 글자14 + 메뉴12 + gap10까지 줄여도 690px로 여전히 초과
  - 로그인/회원가입을 계정 아이콘으로 접으면 672px → 들어감
- 해결
  - 768~1023px: 로고 글자 노출(16px) + 로그인/회원가입은 계정 아이콘 하나로 대체
  - 같은 구간 .global_nav_list gap 18px → 12px (원래 규칙을 직접 수정,
    중복 규칙을 새로 만들지 않음)
  - .header_logo에 min-width: max-content — 로고 칸이 1fr이라 남는 폭만 받아
    글자가 메뉴 위로 5px 넘치던 것을 해결
  - 1024px 이상: 로그인/회원가입 글자 버튼 복원 규칙 추가(원래 없어서 안 돌아왔다)
  - 로그인 상태 페이지는 이메일이 216px라 768~1023px에서 또 넘쳤다.
    css/research.css의 "768px부터 이메일 노출" 규칙을 없애고 1024px부터 노출로 바꿈
- 삽질한 부분
  - 처음에 gap 규칙을 media 블록 앞쪽에 추가했더니 뒤에 있던 gap:18px에 덮였다.
    이메일도 같은 이유로 안 먹혔다. 둘 다 원래 규칙을 직접 고치는 쪽으로 정리
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 375px: 로고 글자 17px 노출, 햄버거 + 계정 아이콘, 넘침 없음
  - 768px: 로고 글자 16px, 메뉴 한 줄, 로고-메뉴 간격 16px, 넘침 없음
  - 900px: 비로그인 간격 22px / 로그인 상태 33px, 둘 다 넘침 없음
  - 1280px: 로고 21px, 로그인/회원가입 글자 복원, 이메일 노출, 넘침 없음
  - 15개 페이지 모두 .header_logo_text = "미래청년비전연구소" 확인
  - 콘솔 오류 0, 가로 스크롤 0
- 확인하지 못한 부분
  - 실기기 테스트

## 파트너십 문구 · 푸터 정리 · 연구분야 호버 · 파비콘 보강 (2026-08-08)

- 완료한 내용
  - index.html 최신 연구보고서 카드 아래에 영문 파트너십 문구 추가(.report_note).
    파란 띠 위라 흰 글씨 78% 불투명도
  - 푸터에서 주소·전화 삭제(15개 페이지). 이메일은 요청에 없어 남겼다
  - 데스크톱 푸터 깨짐 수정: 2열(1fr 1fr) → 3열(auto 1fr auto).
    기존에는 약관 링크가 1열 안에서 오른쪽 정렬돼 좌표 413~620px에 붕 떠 있었다.
    이제 로고 · 이메일 · 약관이 한 줄, 저작권만 아래 줄
  - 연구 분야 카드 호버: 하늘색(--color_brand_light) 안쪽 테두리 2px +
    파란 글로우 + 6px 떠오름, 상단 라인 3→5px, 아이콘/태그 테두리도 하늘색.
    @media (hover: hover) 안에 넣어 터치 기기에서는 적용되지 않는다
  - 파비콘 보강: favicon.ico 신규 생성 후 표준 link 구성으로 교체
- 파비콘 원인 분석
  - 배포본(dmsyjs2539-sys.github.io/youth/)에서 직접 확인한 결과
    favicon.svg / favicon.png 모두 200 + 올바른 MIME으로 서빙되고,
    캔버스로 그려봐도 32x32에 1000/1024 픽셀이 찍혀 파일 자체는 정상이었다
  - 문제는 선언 방식이었다. rel="alternate icon"은 비표준이라 브라우저별로 처리가 다르고,
    /favicon.ico가 없어 폴백도 없었다
  - 32px PNG를 ICO 컨테이너로 감싸 favicon.ico를 만들고(파이썬으로 헤더 직접 작성),
    link를 ico → svg → png 순의 표준 rel="icon" 구성으로 바꿨다
  - ⚠️ 브라우저는 파비콘을 아주 오래 캐시한다. 배포 후에도 안 보이면
    강력 새로고침이나 시크릿 창으로 확인해야 한다
- 검증 결과 (node 정적 서버 http://localhost:4173)
  - 1280px 푸터: 로고(48~247) · 이메일(287~993) · 약관(1033~1232) 한 줄,
    저작권이 전체 폭으로 아래 줄
  - 파트너십 문구 rgba(255,255,255,0.78)로 파란 띠 위 노출 확인
  - 연구 분야 호버 규칙 4개 등록 확인 + 실제 마우스 오버로 하늘색 테두리 육안 확인
  - 15개 페이지 모두 favicon.ico link 보유, 주소/전화 잔재 0건, 없는 에셋 0건
  - 콘솔 오류 0, 가로 스크롤 0
- 확인하지 못한 부분
  - 배포 반영 후 실제 탭 아이콘은 GitHub Pages 재빌드 + 캐시 만료 후 확인 필요
  - 실기기 테스트

## 이전 검증 결과 (2026-08-06, 모바일 메인페이지)

- 실행 명령: node 정적 서버 (http://localhost:4173)
- 결과: 통과
- 확인 화면: 360px, 402px
- 확인 내용:
  - 가로 스크롤 없음(docW=winW), 넘치는 요소 0개, 콘솔 오류 없음, 리소스 전체 200 OK
  - 메뉴 토글 동작 및 aria-expanded / aria-label 갱신, Tab 순서 시각적 흐름과 일치
  - 히어로 스플릿: 22자 3줄 분해, 450ms 시점 opacity 0.77→0.43 순차 그라데이션 확인
  - 햄버거 모핑: 바 1·3 ±45° 회전 + 6.8px 이동, 바 2 scaleX(0.2) 페이드, 닫을 때 완전 복원
  - 메뉴 항목 Stagger: transition-delay 0.09s → 0.35s 순차 적용
  - 스크롤 등장: 최상단에서 뷰포트 내 요소만 발동, 스크롤에 따라 카드 1→2→4개 순차 발동
  - 모션 최소화 규칙 적용 시 숨겨지는 요소 0개
  - 인트로 시퀀스: 0.15s 바디 등장 → 0.35s 마름모 합류(회전+Glow) → 1.35s 정위치 →
    1.4s 심볼 내부 반짝임 스윕 → 1.72s 로고타입 → 2.62s 오버레이 수축 → 3.4s DOM 제거
  - 인트로 심볼·로고타입 모두 뷰포트 중앙(201px)에 정확히 정렬, 영문 태그라인 제거
  - 반짝임이 심볼 실루엣 안에서만 스침 (clipPath 적용 확인)
  - 헤더·푸터·인트로 브랜드명 표기 통일 (당시 확정 명칭 '미래청년비전연구소' 기준.
    2026-08-07에 '미래청년비전연구소'로 되돌림)
  - 360px에서 헤더 로고(우측 178px)와 메뉴 버튼(좌측 310px) 겹침 없음
  - 인트로 사각형 잔상 제거: g의 drop-shadow 제거 전/후 스크린샷 비교로 원인 확정,
    원형 광원 교체 후 사각 경계 사라진 것 확인
  - 인트로 심볼-로고타입 간격 66px → 12px, 로고타입 21px → 26px
  - 맨 위로 버튼: 400px에서 숨김 / 700px에서 표시(pointer-events auto),
    클릭 시 2000px → 0 이동 후 자동 숨김, 360px에서 46x46 뷰포트 내 정상 배치
  - 인트로 종료 후 화면 중앙 elementFromPoint가 hero로 잡혀 클릭 방해 없음 확인
  - .field_card_lead 18px / line-height 27px 적용, 카드 높이 335px → 266px
- 확인하지 못한 부분: 768px·1280px (이번 작업 범위 제외), 실기기 테스트
